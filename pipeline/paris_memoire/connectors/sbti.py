...
Produit ENV_SBTI_VALIDATED. Le cœur anti-greenwashing du pilier ENV :
  ...
  * « Committed » (simple engagement)       -> normalized 1.0, nature=commitment
      (le moteur applique alors le multiplicateur promesse ×0.4 + plafond 0.5)
...
STATUS_COLS = ("near term - target status", "near-term target status", "target status", "status")
...
class SbtiRow:
    ...
    nature: str          # result / commitment
    label: str           # statut d'origine (traçabilité)
...
def parse_csv(path: str) -> list[SbtiRow]:
    """Parse l'export SBTi (CSV, colonnes tolérantes à la casse)."""
    rows: list[SbtiRow] = []
