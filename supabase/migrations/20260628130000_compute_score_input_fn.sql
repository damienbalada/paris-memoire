-- =============================================================================
-- Migration 0007 : fonction d'assemblage des entrées de scoring
-- -----------------------------------------------------------------------------
-- compute_score_input(slug, profile) renvoie en jsonb tout ce dont le moteur
-- (scoring.ts) a besoin : chaîne d'ancêtres (résolution marque→groupe avec
-- specificity), indicateurs applicables au secteur, evidence active héritée,
-- valeurs des pairs pour la normalisation intra-secteur, profil, et config
-- méthodo. La logique de CALCUL reste hors SQL (TypeScript pur, testé).
-- Utilisée pour la validation hors-ligne et réutilisable comme RPC par l'Edge
-- Function pour alléger sa couche d'accès aux données.
-- =============================================================================
...
appl as (
  select i.id, i.code, d.code as dimension_code, i.kind, i.direction, i.weight
...
select jsonb_build_object(
  ...
  'applicableIndicators', (select coalesce(jsonb_agg(jsonb_build_object(
      'code', code, 'dimension_code', dimension_code, 'kind', kind, 'direction', direction, 'weight', weight)), '[]') from appl),
