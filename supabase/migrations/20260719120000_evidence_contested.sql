-- =============================================================================
-- Règle 4 (corroboration) : flag `contested` sur l'evidence.
-- Une controverse issue d'une source explicitement contestée (ex : rapport isolé
-- d'un mandat-holder) est AFFICHÉE mais EXCLUE du calcul tant qu'elle n'est pas
-- corroborée (≥ 2 sources distinctes ou décision de justice). Le moteur applique
-- la logique ; ici on ajoute la colonne et on la transmet à compute_score_input.
-- =============================================================================

alter table evidence add column if not exists contested boolean not null default false;

-- La vue evidence_active énumère ses colonnes : on la recrée pour exposer `contested`.
create or replace view evidence_active as
  select id, entity_id, indicator_id, source_id, value_type, value_numeric, value_boolean,
         value_text, normalized_value, nature, observed_on, valid_until, confidence, reviewer,
         review_status, excerpt, source_url, created_at, updated_at, contested
  from evidence e
  where review_status = 'approved'::review_status and valid_until >= current_date;
...
-- Marque comme contestés les faits issus du rapport de la Rapporteuse spéciale ONU.
update evidence ev set contested = true
from sources s
where ev.source_id = s.id and s.code = 'UN_SR_OPT';
