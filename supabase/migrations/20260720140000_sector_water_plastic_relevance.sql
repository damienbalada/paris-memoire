-- =============================================================================
-- Découplage de la pertinence Eau (WAT) et Plastique (PLA) par secteur.
-- -----------------------------------------------------------------------------
-- Depuis la scission du pilier WAP en PLA + WAT, un seul flag `wap_relevant`
-- pilotait les DEUX — ce qui excluait à tort l'Eau pour des secteurs très
-- consommateurs d'eau mais non concernés par le plastique d'emballage
-- (ex : automobile, énergie). On sépare en `plastic_relevant` + `water_relevant`.
--   • PLA (audits Break Free From Plastic, Global Commitment) = plastique
--     d'emballage grand public → non pertinent pour l'auto.
--   • WAT (CDP Water, intensité, stress hydrique) = eau industrielle →
--     pertinent pour l'auto et l'énergie.
-- =============================================================================

alter table sectors add column if not exists plastic_relevant boolean not null default false;
alter table sectors add column if not exists water_relevant   boolean not null default false;

-- Backfill : on repart de l'ancien flag unique, puis on corrige les cas eau.
update sectors set plastic_relevant = wap_relevant, water_relevant = wap_relevant;
update sectors set water_relevant = true where code in ('automotive','energy');

-- compute_score_input : gate PLA et WAT séparément.
...
me_wap as (
  select coalesce((select s.plastic_relevant from sectors s where s.id = (select sector_id from me)), false) as plastic_relevant,
         coalesce((select s.water_relevant   from sectors s where s.id = (select sector_id from me)), false) as water_relevant
),
me_flags as (
...
appl as (
  ...
  where (i.sector_specific = false or i.applies_to_sector_id = (select sector_id from me))
    ...
    and (d.code <> 'PLA' or (select plastic_relevant from me_wap))
    and (d.code <> 'WAT' or (select water_relevant from me_wap))
),
ev as (
  select a.code as indicator_code, s.tier, e.nature, e.normalized_value, e.value_numeric,
         e.confidence, e.observed_on, sp.specificity, e.contested, s.code as source_code,
...
select jsonb_build_object(
  ...
  'evidence', (select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'specificity', specificity, 'contested', contested, 'source_code', source_code, 'source_url', source_url)), '[]') from ev),
